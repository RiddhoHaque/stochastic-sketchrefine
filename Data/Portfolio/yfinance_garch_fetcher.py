...
def _fit_garch(log_returns: np.ndarray):
    """Fit GARCH(1,1) to daily log returns scaled by 100 for numerical stability."""
    ...
    cond_var = cond_vol ** 2
    std_resids = (log_returns - float(res.params.get('mu', 0.0)) / 100) / cond_vol
    last_h      = float(cond_var[-1])
    last_return = float(log_returns[-1])
    return omega, alpha1, beta1, last_h, last_return, std_resids
...
def _insert_ticker(cursor, year: int, row_ids_param: dict, row_ids_resid: dict,
                   ticker: str, price: float,
                   omega: float, alpha1: float, beta1: float,
                   last_h: float, last_return: float,
                   residuals: np.ndarray, valid_sell_afters: list):
    param_table = f'GARCH_Portfolio_{year}'
    resid_table = f'GARCH_Residuals_{year}'

    # Parameter rows — one per valid sell_after value
    row_id = row_ids_param[year]
    for sell_after in valid_sell_afters:
        cursor.execute(
            f'INSERT INTO {param_table} VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s);',
            (row_id, ticker, float(sell_after), price,
             omega, alpha1, beta1, last_h, last_return),
        )
        row_id += 1
    row_ids_param[year] = row_id

    # Residual rows — bulk insert via execute_values
    row_id = row_ids_resid[year]
    resid_rows = []
    for z in residuals:
        if math.isnan(z) or math.isinf(z):
            continue
        resid_rows.append((row_id, ticker, float(z)))
        row_id += 1
    if resid_rows:
        psycopg2.extras.execute_values(
            cursor,
            f'INSERT INTO {resid_table} (id, ticker, residual_value) VALUES %s',
            resid_rows,
        )
    row_ids_resid[year] = row_id
...
def main():
    ...
    row_ids_param = {year: 0 for year in CUTOFF_YEARS}
    row_ids_resid = {year: 0 for year in CUTOFF_YEARS}
    ...
    for i, ticker in enumerate(tickers):
        try:
            ...
            for year in CUTOFF_YEARS:
                saved_param = row_ids_param[year]
                saved_resid = row_ids_resid[year]
                try:
                    ...
                    # Fit GARCH on historical data up to cutoff
                    cutoff  = cutoffs[year]
                    ...
                    omega, alpha1, beta1, last_h, last_return, residuals = _fit_garch(log_returns)

                    if any(math.isnan(v) or math.isinf(v)
                           for v in [omega, alpha1, beta1, last_h, last_return]):
                        cursor.execute('RELEASE SAVEPOINT sp_year')
                        continue
                    ...
                    _insert_ticker(cursor, year, row_ids_param, row_ids_resid,
                                   ticker, price, omega, alpha1, beta1,
                                   last_h, last_return, residuals, valid_sa)
                    cursor.execute('RELEASE SAVEPOINT sp_year')

                except Exception as e:
                    cursor.execute('ROLLBACK TO SAVEPOINT sp_year')
                    row_ids_param[year] = saved_param
                    row_ids_resid[year] = saved_resid
    ...
    conn.commit()
    for year in CUTOFF_YEARS:
        print(f'  GARCH_Portfolio_{year}: {row_ids_param[year]} param rows, '
              f'{row_ids_resid[year]} residual rows.')
    cursor.close()
    conn.close()
    print('Done.')
