...
and random initial Z constraints (set_initial_constraints_randomly=True).

Runs two configurations for comparison:
  1. LP-first, heuristic initial constraints  (lowest total scenario values)
  2. LP-first, random initial constraints
...
CONFIGS = [
    ('LP_First / heuristic_init',
     {'solve_lp_first': True,
      'set_initial_constraints_randomly': False}),
    ('LP_First / random_init',
     {'solve_lp_first': True,
      'set_initial_constraints_randomly': True}),
]
...
def main():
    ...
    out_path = os.path.join(eval_dir, 'benchmark_random_initial_constraints.txt')
