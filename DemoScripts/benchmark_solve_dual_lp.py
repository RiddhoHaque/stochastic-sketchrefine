#!/usr/bin/env python3
"""
Benchmark script: solve Q2.sql with RCLSolve using iterative constraint addition
and LP-first, comparing solve_dual_lp=True (dual simplex) vs solve_dual_lp=False
(primal simplex).

Both runs use:
    iterative_constraint_addition=True
    solve_lp_first=True
    use_gpu=False   (so the LP method choice is not overridden by PDHG)
...
CONFIGS = [
    ('LP_First / solve_dual_lp=True  (dual simplex)',
     {'solve_lp_first': True, 'use_gpu': False, 'solve_dual_lp': True}),
    ('LP_First / solve_dual_lp=False (primal simplex)',
     {'solve_lp_first': True, 'use_gpu': False, 'solve_dual_lp': False}),
]
...
def main():
    ...
    out_path = os.path.join(eval_dir, 'benchmark_solve_dual_lp.txt')
